...
Hey, this still doesn't work. 
You also need to load packages. You could use option `.packages` of `foreach` but you could simply add `dplyr::` before `count`. It will be clearer (like one does in packages).
...
You may also need to use some message passing or some barriers. For that, you could learn to use MPI. For some basic use, I "reimplemented" this using only shared-memory matrices (FBMs). You can see [this function](https://github.com/privefl/bigstatsr/blob/master/R/randomSVD.R#L4-L91) is you're interested.
