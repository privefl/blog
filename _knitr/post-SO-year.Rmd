...
myRep %>%
  ...
  ggtitle("Evolution of my SO reputation over the last year") + 
  bigstatsr::theme_bigstatsr()
```

So, it seems that my activity is slowing gently (my reputation is proportional to the square root of time). Yet, it is still increasing steadily; so what is my strategy for answering questions on SO?

### Analyzing my answers

You'll have to wait for the answer to what is my strategy for answering questions on SO. For a hint, let's analyze my answers and tags I'm involved in.

```{r}
(myAnswers <- stack_users(myID, "answers", num_pages = 30,
                          fromdate = today() - years(1)) %>%
...
So it seems I've answered `r nrow(myAnswers)` questions over the past year. This time, my answers with the greatest scores were mostly accepted. 
...
mean(myAnswers$score)
```

It seems that these days, it is harder to get answers that are highly upvoted. 
...
- The tags I'm mostly answering questions from are "performance", "rcpp", "parallel-processing", "foreach", "r-bigmemory", "vectorization". 
...
I can spend hours on a question about performance and am sometimes rewarded with a solution that is 3 order of magnitude faster (see e.g. [this other post](https://privefl.github.io/blog/performance-when-algorithmics-meets-mathematics/)).
